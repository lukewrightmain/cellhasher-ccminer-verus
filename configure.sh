...
./configure CXXFLAGS="-Rpass-missed=loop-vectorize -Rpass-analysis=loop-vectorize $extracflags -march=armv8.6-a+crypto+sha3+sm4+dotprod+i8mm+bf16 -mtune=cortex-a710" CFLAGS="$extracflags -march=armv8.6-a+crypto+sha3+sm4+dotprod+i8mm+bf16 -mtune=cortex-x2 -mllvm -enable-loop-distribute" CXX=clang++ CC=clang LDFLAGS="-flto -fuse-ld=lld -pthread"
